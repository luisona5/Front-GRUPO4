...
import { Link, useNavigate } from 'react-router'; // corregido a react-router-dom
...
const Login = () => {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (response && response.token) {
        setToken(response.token);
        setRol(response.rol || 'administrador'); // rol por defecto administrador
        navigate('/dashboard');
      } else {
  ...
  return (
    <div className="flex flex-col sm:flex-row h-screen">
      ...
      {/* Imagen de fondo */}
      <div 
        className="w-full sm:w-1/2 h-1/3 sm:h-screen bg-[url('/public/images/basquet.jpg')] bg-no-repeat bg-cover bg-center sm:block hidden"
      ></div>
      ...
      <div className="w-full sm:w-1/2 h-screen bg-white flex justify-center items-center">
        <div className="md:w-4/5 sm:w-full">
          ...
          <div className="mt-3 text-sm flex justify-between items-center">
            <Link to="/" className="underline text-sm text-gray-400 hover:text-gray-900">
              Regresar
            </Link>
            <Link to="/register" className="py-2 px-5 bg-gray-600 text-slate-300 border rounded-xl hover:scale-110 duration-300 hover:bg-gray-900 hover:text-white">
              Registrarse
            </Link>
          </div>
        </div>
